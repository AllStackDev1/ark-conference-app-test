...
) => await post<FormResponse<IAttendee>>(`/conferences/${id}/attendee/register`, JSON.stringify(payload));

export const addTalkToConference = async (
  id: string,
  payload: IAddTalkPayload
) => await post<FormResponse<IConference>>(`/conferences/${id}/talk`, JSON.stringify(payload));
